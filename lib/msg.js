...
let userWarns = {};  // Dictionnaire pour stocker les avertissements par utilisateur

// Fonction antilink avec gestion des avertissements
const antilink = async (m) => {
    ...
    if (urlRegex.test(m.body)) {
        ...
        conn.sendMessage(userId, { text: `Avertissement #${userWarns[userId]} : Vous avez envoyé un lien, ce n'est pas permis.` });

        // Si l'utilisateur a 3 avertissements, envoie un message spécial
        if (userWarns[userId] >= 3) {
            conn.sendMessage(userId, { text: "Vous avez reçu 3 avertissements. Si vous continuez, vous serez expulsé." });
        }

        return true;  // Signale que l'antilink a détecté et traité un lien
    }
    return false;  // Si aucun lien n'est détecté, retourne false
}
